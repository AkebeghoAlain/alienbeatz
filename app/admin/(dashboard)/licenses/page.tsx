          ...
          <div key={beat.id} className="glass rounded-lg p-5">
            ...
            <div className="grid gap-4">
              {beat.licenses.map((license) => (
                <form key={license.id} action={updateLicense} className="grid gap-3 rounded-md border border-white/10 p-4 md:grid-cols-[160px_140px_1fr_auto] md:items-center">
                  <input type="hidden" name="id" value={license.id} />
                  <p className="font-semibold text-white">{license.license_name}</p>
                  <input name="price" type="number" defaultValue={license.price} className="h-11 rounded-md border border-white/10 bg-black/25 px-3 text-sm text-white outline-none" />
